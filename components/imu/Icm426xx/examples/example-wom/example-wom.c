...
int GetDataFromInvDevice(void)
{
	...
	if (int_status &
	    (BIT_INT_STATUS2_WOM_X_INT | BIT_INT_STATUS2_WOM_Y_INT | BIT_INT_STATUS2_WOM_Z_INT)) {
		/*
		...
		wom_flag = int_status & (BIT_INT_STATUS2_WOM_X_INT | BIT_INT_STATUS2_WOM_Y_INT |
		                         BIT_INT_STATUS2_WOM_Z_INT);

		INV_MSG(INV_MSG_LEVEL_INFO, "WoM interrupt at %u (X, Y, Z): %d, %d, %d",
		        (uint32_t)irq_timestamp, (wom_flag & BIT_INT_STATUS2_WOM_X_INT) ? 1 : 0,
