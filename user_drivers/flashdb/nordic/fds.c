...
static volatile uint32_t     m_latest_rec_id;

static uint32_t m_users;

static fds_op_t m_fds_op;

// Internal status flags.
static struct
{
	bool volatile     initialized;
	volatile uint32_t initializing;
} m_flags;
 ...
 */
__STATIC_INLINE bool is_double_word_aligned(void const* p)
{
...
static ret_code_t record_header_write_begin(fds_op_t* const p_op, uint32_t* const p_addr)
{
	...
	ret = nrf_fstorage_write(&m_fs,
		(uint32_t)(p_addr + FDS_OFFSET_TL),
		&p_op->write.header.record_key,
		FDS_HEADER_SIZE_TL * sizeof(uint32_t),
		NULL);
...
static ret_code_t record_header_write_id(fds_op_t* const p_op, uint32_t* const p_addr)
{
	...
	ret = nrf_fstorage_write(&m_fs,
		(uint32_t)(p_addr + FDS_OFFSET_ID),
		&p_op->write.header.record_id,
		FDS_HEADER_SIZE_ID * sizeof(uint32_t),
		NULL);
...
static ret_code_t record_header_write_finalize(fds_op_t* const p_op, uint32_t* const p_addr)
{
	...
	ret = nrf_fstorage_write(&m_fs,
		(uint32_t)(p_addr + FDS_OFFSET_IC),
		&p_op->write.header.file_id,
		FDS_HEADER_SIZE_IC * sizeof(uint32_t),
		NULL);
...
static ret_code_t record_header_flag_dirty(uint32_t* const p_record, uint16_t page_to_gc)
{
	...
	__ALIGNED(4) static uint32_t const dirty_header[2] = { 0xFFFF0000, 0x00000000 };
...
static ret_code_t record_write_data(fds_op_t* const p_op, uint32_t* const p_addr)
{
	...
	ret = nrf_fstorage_write(&m_fs,
		(uint32_t)(p_addr + FDS_OFFSET_DATA),
		p_op->write.p_data,
		p_op->write.header.length_words * sizeof(uint32_t),
		NULL);
...
static ret_code_t gc_record_copy(void)
{
	...
	return nrf_fstorage_write(&m_fs,
		(uint32_t)p_dest,
		m_gc.p_record_src,
...
static void queue_process(ret_code_t result)
{
	//while(true)
	{
		fds_op_t* m_p_cur_op = &m_fds_op;      // Current fds operation.
	...
	fds_op_code_t                     op_code)
{
	...
	crc = crc16_compute((uint8_t*)p_record->data.p_data,
		p_record->data.length_words * sizeof(uint32_t),
		&crc);
#endif
