 ...
 */
void
md5_update(MD5_CTX* mdContext, unsigned char* inBuf, unsigned int inLen)
{
	uint32_t in[16];
	int mdi;
	unsigned int i, ii;
...
#if 0
	PPPDEBUG(LOG_INFO, ("md5_update: %u:%.*H\n", inLen, LWIP_MIN(inLen, 20) * 2, inBuf));
	PPPDEBUG(LOG_INFO, ("md5_update: %u:%s\n", inLen, inBuf));
#endif
 ...
 */
void
md5_final(unsigned char hash[], MD5_CTX* mdContext)
{
	uint32_t in[16];
	int mdi;
	unsigned int i, ii;
...
void md5_get_hash_value(uint8_t inBuf[], uint32_t inLen, uint8_t ouput[])
{
	MD5_CTX md5;
	md5_init(&md5);
	md5_update(&md5, inBuf, inLen);
	md5_final(ouput, &md5);
}
